...
/*   Updated: 2024/05/01 20:11:25 by sei              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_file(char *buffer, t_tknlist *list, t_tkntype type)
{
	...
	if (type == APPEND || type == HEREDOC)
		...
		i = 1;
	if (!add_node(list, create_node(type, gc_strndup(buffer, i, TKN_LIST), 1)))
...
{
	if (!add_node(list, create_node(PIPE, gc_strndup(buffer, 1, TKN_LIST), 0)))
		print_and_exit(ERR_MALLOC, RED, 1);
	return (1);
}
...
{
	if (!add_node(list, create_node(type, gc_strndup(buffer, 2, TKN_LIST), 0)))
		print_and_exit(ERR_MALLOC, RED, 1);
	return (2);
}
...
int	handle_s_quote(char *buffer, t_tknlist *list)
{
	...
	if (ft_isspace(buffer[i + 1]) == 0)
		link = 1;
	if (!add_node(list,
			create_node(S_QUOTE, gc_strndup(buffer, i + 1, TKN_LIST), link)))
		print_and_exit(ERR_MALLOC, RED, 1);
	return (i + 1);
}
...
int	handle_d_quote(char *buffer, t_tknlist *list)
{
	...
	if (ft_isspace(buffer[i + 1]) == 0)
		link = 1;
	if (!add_node(list,
			create_node(D_QUOTE, gc_strndup(buffer, i + 1, TKN_LIST), link)))
		print_and_exit(ERR_MALLOC, RED, 1);
	return (i + 1);
}
